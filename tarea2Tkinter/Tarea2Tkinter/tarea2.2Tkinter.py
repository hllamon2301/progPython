...
class Aplicacion:
    def __init__(self):
        ...
        self.label3.grid(column=2, row=4)
    ...
    def operar(self):
        ...
        if self.seleccion.get()==3:
            producto=int(self.dato1.get())*int(self.dato2.get())
            self.label3.configure(text=producto)
        if self.seleccion.get()==4:
            división=int(self.dato1.get())/int(self.dato2.get())
            self.label3.configure(text=división)
